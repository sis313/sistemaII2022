...
public class CUSTOMService {
        ...
        public List<BranchRatingCountDto> findBranchRatingCountDto() {
                @SuppressWarnings("unchecked")
                List<BranchRatingCountDto> branchRatingCountDtos = (List<BranchRatingCountDto>) entityManager
                                .createNativeQuery(
        ...
        public Integer updateBusinessStatusDto(Integer status, Integer idUser) {
                Integer countUpdate = entityManager
                                .createNativeQuery(
                                                "UPDATE business SET business.status = :status WHERE business.id_user = :idUser ;")
        ...
        public List<LogSemesterCountDto> findLogSemesterCountDto() {
                @SuppressWarnings("unchecked")
                List<LogSemesterCountDto> logSemesterCountDtos = (List<LogSemesterCountDto>) entityManager
                                .createNativeQuery(
        ...
        public List<LogQuarterCountDto> findLogQuarterCountDto() {
                @SuppressWarnings("unchecked")
                List<LogQuarterCountDto> logQuarterCountDtos = (List<LogQuarterCountDto>) entityManager
                                .createNativeQuery(
        ...
        @Transactional
        public RatingAverageDto findBusinessRatingAverageDto(Integer businessId) {
                RatingAverageDto ratingAverageDto = (RatingAverageDto) entityManager
                                .createNativeQuery(
                                                "SELECT AVG(ra.score) AS averageScore FROM branch AS br, rating AS ra WHERE br.id_branch = ra.id_branch AND br.id_business = :businessId ;",
                                                "BusinessRatingCount")
